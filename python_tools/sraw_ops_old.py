...
Obiettivi:
- evitare array PHP enormi in RAM
- usare sraw.conf tramite get_constants()
- lavorare su griglie dense NumPy compatte
- interpretare i file SRAW con:
...
Note:
- sum / mul producono output symmetric se almeno uno dei due input è symmetric
- mirror_y produce sempre output symmetric
- dilate_x mantiene axis_mode di A se factor > 0, altrimenti usa symmetric
"""
...
def stream_read_dense_sraw(filepath: str, C: dict):
    """
    Legge un file SRAW senza costruire una lista gigantesca di tuple Python.
    Ricostruisce direttamente due array dense (re, im) su griglia intera fissa.

    Semantica:
    - tra due punti espliciti -> interpolazione lineare
    - fuori dal supporto noto -> zero
    - punti duplicati con stesso x -> vale l'ultimo
    """
    N = C["MAX_SAMPLES"]
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        # leggiamo prima axis_mode
        # poi ricostruiamo direttamente gli array
        buffered_lines = []
        ...
        # ora continuiamo a leggere i campioni
        only_one_point = None
        ...
        for line in f:
            ...
            # duplicato sullo stesso x: vince l'ultimo
            if cur[0] == prev[0]:
                prev = cur
                only_one_point = cur
                continue
        ...
        # caso degenerato: un solo punto esplicito nel file
...
def write_dense_sraw(filepath: str, axis_mode: str, re: np.ndarray, im: np.ndarray, C: dict, comment: str = ""):
    ...
    if len(re) != (xmax - xmin + 1) or len(im) != (xmax - xmin + 1):
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        x = xmin
        for r, j in zip(re_i, im_i):
            f.write(f"{x},{int(r)},{int(j)}\n")
            x += 1
...
def run(
    ...
    if op == "sum":
        ...
        # riscalatura per restare nello stesso “ordine di grandezza” SRAW
    ...
    if verbose or benchmark:
        print(
            f"[sraw_ops] op={op}, N={N}, axis_out={out_axis_mode}, time={elapsed:.4f}s"
        )
